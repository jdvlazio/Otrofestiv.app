...
- [x] 1. `python3 validate.py` → 26/26 baseline + `node --test tests/unit/*.test.js` → 131/131
- [x] 2. Crear branch `refactor/subscribe-render-7d`
- [x] 3. **Mapa de dependencias** — examinar cada handler de core slice y
...
- [x] 4. QA browser PRE: trap de errors + festival cargado + render counter instrumentado.
- [x] 5. **State container: subscribeRender + _runRenderSubs** (entre `subscribe`
...
- [x] 6. **Unit test dedup** — extender `state.test.js` (sandbox makeSandbox):
...
- [x] 7. **renderActiveView() router COMPREHENSIVE** (Q1=A) — encapsula la matriz
      (activeView × activeMNav) + cache-bust + scroll/pel-guard + runCalc-planner:
      ```js
      function renderActiveView() {
        cachedResult = null;                      // state cambió → cache stale
        ...
        if (activeMNav === 'mnav-planner') { runCalc(); return; }  // Q2: pipeline llama runCalc (recompute+render)
        renderAgenda();                           // rutea internamente seleccion/miplan
      }
      ```
      Q3: `_reRenderIntereses` se MANTIENE surgical (no se absorbe) — los handlers
      que lo llaman lo conservan (igual que updateCardState).
- [x] 8. **RENDER PIPELINE registrations** (sección nueva, según mapa de paso 3):
      ```js
      // ── RENDER PIPELINE (p7d) ──
      ...
      ```
- [x] 9. **Wave 1 — remover render calls de handlers watchlist/watched/prioritized/filmRatings**:
...
- [x] 10. **Wave 2 — remover render calls de handlers schedule slices**:
...
- [x] 11. **Wave 3 — remover render calls de handlers availability**:
...
- [x] 14. `python3 validate.py` → **26/26**
- [x] 15. `node --test tests/unit/*.test.js` → **~135/135** (131 + nuevos)
- [x] 16. JS syntax check vía extracción + Function constructor → OK
- [x] 17. **Functional equivalence (R2')** por slice (browser):
