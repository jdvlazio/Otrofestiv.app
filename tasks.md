...
- [x] `genre` i18n — almacenado solo en ES; usuarios EN/PT ven el género en español (aplica a los 40, no bloqueante, post-festival).
- [x] `lbSlug` — **cobertura completa: 46/46 films solos traen campo `lbSlug`** (0 vacíos; lo consume el sheet vía `sheets-controller.js`). Pendiente: verificación per-slug (método Chrome tab) — la correctitud de cada slug no está verificada. Nota: el objeto root `lbSlugs` está vacío (la fuente real es el campo per-film).
...
- [x] `tools/enricher.html` — agregar chip UI `data-fest="olhar2026"` (chips hardcodeados, ver tribeca L64). **Polish post-festival, no bloqueante**: el flujo canónico de enrichment es CLI (`scripts/enrich-festival.py`, Fase 3); el chip es solo conveniencia de la UI.
- [x] Pósters editoriales de los 12 programas — **NO BLOQUEANTE** (12/12 ya resueltos vía generativo `makeProgramPoster`/`_buildPosterV16`, ninguno roto). 0/12 con póster explícito en el JSON; editorial/custom es opcional.
