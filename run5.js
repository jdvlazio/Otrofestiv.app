...
console.log('plan storage:',await p.evaluate(()=>Object.keys(localStorage).filter(k=>/finca/.test(k)).map(k=>k+'='+localStorage.getItem(k))));
...
await p.screenshot({path:SHOT+'30-miplan-top.png'});
for(let i=1;i<=5;i++){await p.mouse.wheel(0,650);await p.waitForTimeout(700);await p.screenshot({path:SHOT+'30-miplan-'+i+'.png'});}
