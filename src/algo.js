...
function computeScenarios(titles){
  ...
  // Solo títulos con funciones futuras pueden aparecer en excluded
  const eligibleTitles=new Set(baseGroups.map(g=>g.title));
  ...
  return allScenarios.map(sc=>{
    ...
    return{
      schedule:sc,
      excluded:pending.filter(t=>eligibleTitles.has(t)&&!included.has(t)),
