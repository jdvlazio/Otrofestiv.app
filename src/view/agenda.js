      ...
      listHtml+=`<div class="mplan-row${_rowKey===_activeMiPlanFilm?' active':''}${isSeen?' mp-seen':''}" style="cursor:pointer" data-rkey="${_safeRowKey}" data-action="selectFromDetail">
        ...
        <div class="col-end">
          ${isSeen
            ...
            :`<button class="icon-btn-circle ag-fi-btn del" data-title="${safeT}" data-day="${s.day||''}" data-time="${s.time||''}" data-action="removeFromAgenda" data-stop="1" aria-label="${t('plan_sacar')}">${ICONS.x}</button>`}
        </div>
...
export function renderFilmListHTML(state){
  ...
  function _mkItem(title){
    ...
    // `short||venue`: sin el respaldo, una sede sin `short` pintaba la palabra
