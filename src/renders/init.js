...
async function loadFestival(id){
  ...
  // Fetch TMDB posters para films con tmdb_id — secuencial para evitar race conditions
  const _tmdbFilms=(cfg.films||[]).filter(f=>f.tmdb_id&&!cfg.posters[f.title]);
  if(_tmdbFilms.length){
    (async()=>{
      let updated=false;
      for(const f of _tmdbFilms){
        try{
          ...
          if(r.ok){
            ...
            if(d.poster_path){
              ...
              cfg.posters[f.title]=url;
              POSTERS[f.title]=url;
              _POSTERS_N[normKey(f.title)]=d.poster_path;
              updated=true;
            }
          }
        }catch(e){}
      }
      if(updated) _renderProgramaContent();
    })();
  }
