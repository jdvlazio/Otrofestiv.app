...
|---|---|---|---|---|
| `title` | string | — | 19 fest | Nombre oficial, verbatim del festival. La palabra la pone el festival. |
| `section` | string | — | 19 fest | Nombre VERBATIM del festival + nuestro emoji. Arquetipo de los 9 canónicos. |
| `day` | string | `^\d{4}-\d{2}-\d{2}$` | 19 fest | Clave exacta de dayKeys, en ISO. Los 5 festivales legacy usan «MAR 21» y quedan exentos. |
| `time` | string | `^\d{2}:\d{2}$` | 19 fest | 24h con dos dígitos. Nunca 12h con AM/PM. |
| `venue` | string | ` - .+$` | 19 fest | «Nombre de la Sede - Ciudad», SIEMPRE. La sala va en `sala`, nunca en el nombre. |
| `day_order` | number | — | 19 fest | **derivado de `day`** — no viene de ninguna fuente Orden del día en la grilla. |
