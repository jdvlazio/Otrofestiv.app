...
| `siembrafest2026` | SiembraFest | Villeta | 9–18 SEP | **Próximo / activo** |
...
- **i18n:** la fuente de verdad es `src/i18n/i18n.js` (bloque `_I18N`, es+en). Toda string nueva va ahí — es lo que lee `t()` y lo que valida `validate.py [i18n-complete]`. Los `i18n/*.json` legacy ya no existen (retirados; medido 5 sep 2026: 0 archivos). El `sync-i18n.py` fue retirado (apuntaba a un `_I18N` en `index.html` que la Fase 8 movió a `src/i18n/i18n.js`).
