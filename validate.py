...
# hay registro de cuándo. Los tres se exigen. `kind` admite 'postponed' y 'moved'
# (QAFF 2026, que trasladó su edición entera a Bogotá y SÍ se hace, en sus fechas):
...
try:
    ...
    for _i, _fid in enumerate(_ids):
        ...
        _KINDS = ('postponed', 'moved')
        if not _k or _k.group(1) not in _KINDS:
            _malos.append(f"{_fid}: kind {_k.group(1)!r} desconocido (solo {_KINDS}) — la banda NO se pintaría y el cambio quedaría sin explicar" if _k else f'{_fid}: status sin kind')
            continue
...
try:
    ...
    _ALLOW = {
        ...
        'src/view/helpers.js': 1166,  # +2: la píldora del distintivo (APERTURA/CLAUSURA/ESTRENO) en _metaBadges — el campo `premiere` lo llevaban 170 funciones de cuatro festivales sin que ninguna vista lo pintara (23 sep)  # antes 1164,  # +9: la Escalera pasa a dibujarse con las obras que TIENEN afiche —decisión de Juan, 16 sep, medida antes de tocarla: 112 → 143 programas con pila— y las nueve líneas son el porqué de revertir su propia regla anterior («solo completa»), que sin el porqué vuelve a escribirse sola  # antes 1155,  # +26: icsHuella + icsSeq — la versión de un evento (SEQUENCE) se decide comparando la firma de lo que puede cambiar SIN cambiar el UID (11 sep)  # antes 1129,  # +38: icsCampos — dueño único de «qué va en este evento»; el puente nativo recalculaba inicio y fin por su cuenta con un comentario que prometía que coincidían, y versionar el evento exige firmar lo MISMO que se manda (11 sep)  # antes 1091,  # +17: icsFantasmas + _icsEntrega — la memoria de entregas pasa a guardar título/día/hora para poder NOMBRAR lo colgado, no solo contarlo (11 sep)  # antes 1074,  # +50: icsUid/icsNuevas/_ics24h/_icsUtc — dueño único de «cómo se llama esta función en un calendario»; lo consultan el .ics, la resta de lo ya entregado y el puente nativo de iOS (10 sep)  # antes 1024,  # +4: el comentario del dueño del póster decía «siempre generativo, ignora f.poster» y su propia línea decía lo contrario — 4 líneas de porqué valen menos que 57 eventos sin su afiche (8 sep)  # antes 1020,  # +20: abiertaLabel/abiertaRango — la actividad ABIERTA dice su fase donde las demás dicen su duración; viven junto a durFmt, que es su par (7 sep)  # antes 1000,  # +6: la fusión de la rama de QAFF con main — las dos tocaron el archivo y el techo cuenta el resultado (4 sep)  # antes 994,  # +7: la cuenta del cruce marca el fin como estimado si la duración no está publicada (4 sep)  # antes 987,  # +18: hayEvento pasa a ser dueño único del sustantivo (obras vs actividades), con su porqué (2 sep)  # antes 969,  # +8: festivalCities marca la ciudad cuya programación cayó entera — dueño único de la hoja de apertura y del filtro de Lugar (Juan, 2 sep)  # antes 961,  # +7: el interruptor de Prensa entra a planInputSignature — apagarlo dejaba el Plan en un pase que ya no existía — 30 ago  # antes 954,  # +17: la LISTA pregunta al mismo dueño que el grid — 207 de 215 compuestos son is_cortos y el gate viejo los dejaba fuera — 26 ago  # antes 926,  # +28: getFilmPosterMini + cableado de lista/thumb/stack a la mini — 25 ago  # antes 898,  # +14: rótulo/firma en los llamadores + halo en el póster grande — 24 ago  # +1: el camino #8 pasa el dato compuesto — 24 ago  # +6: badge PRENSA en _metaBadges — 23 ago  # +17: legacyProgramParts — el programa «A + B» usa la forma C — 21 ago  # +5: la sección nunca se pinta con fill undefined — 19 ago
        'src/view/agenda.js': 2318,  # +12: ojo «no la vi» y nota en el Diario (24 sep)  # antes 2306,  # +18: retraso v2 — barra con la espera, horas reales y control − / + en la tarjeta; bloque corrido en el calendario (25 sep)  # antes 2288,  # +13: «Salís HH:MM» en la fila y el contorno de lo que te perdés en el calendario (25 sep)  # antes 2275,  # +5: el Plan vivo de Sugerencias es lo que no TERMINÓ, no lo que «ya no llegás» — la función en curso no deja hueco (20 sep)  # antes 2270,  # +8: el aviso de lo que quedó colgado en el calendario — se nombra porque no se puede retractar (11 sep)  # antes 2262,  # +14: el botón de exportar con dos estados — con memoria de lo entregado y plan crecido ofrece la resta (10 sep)  # antes 2248,  # -1: la fila de Disponibilidad se delega a su dueño (renderAvFilaHTML) y renderAvBlocks pinta la superficie entera (10 sep)  # antes 2249,  # +18: la fila de Disponibilidad con DOS estados — vacía ofrece (la pregunta sale del sheet) y con bloques encabeza; el porqué medido va en el comentario porque es la razón por la que la usuaria de TIFF guardó un plan sin verla (10 sep)  # antes 2249,  # +18: la actividad ABIERTA en Planear (aparte del contador, vacío honesto) y en Mi Plan (carril bajo la cabecera + fila «Sin hora fija») (7 sep)  # antes 2231,  # +15: el titular del Plan nombra la ciudad cuando el filtro la restringe (4 sep)  # antes 2216,  # +8: la fila de Intereses deja de decir «Vista» dos veces — el dato muestra la sección (4 sep)  # antes 2208,  # +13: «qué viste» se pregunta al dueño único en las 4 puertas del modo Recuerdo (auditoría 4 sep)  # antes 2195,  # +4: la hora de salida se marca estimada cuando la duración no está publicada (4 sep)  # antes 2191,
        'src/main.js': 1849,  # +2: acciones del ojo y la nota del Diario (24 sep)  # antes 1847,  # +1: acción quedarmeHastaFinal — «Ir a las dos» (25 sep)  # antes 1846,  # +16: ResizeObserver sobre .topbar — la variable sticky SIGUE al chrome en vez de fijarse en sitios a mano; la franja bajo TRASLADADO en QAFF (captura de Juan, 15 sep) era la variable vieja tras encoger el chrome  # antes 1830,  # +1: olvidarFantasmas en el despachador (11 sep)  # antes 1829,  # +14: `?simTime=` fija el reloj ANTES del arranque — sin eso lo que el boot decide mirando la hora quedaba fuera de toda prueba (4 sep)  # antes 1815,  # +6: el que abre la ficha no se veta a sí mismo con data-stop — el póster de Mi Plan no abría (4 sep)  # antes 1809,  # +10: #dbg-ver sale del BUILD_VERSION que corre, no de un literal de mayo — 31 ago  # antes 1799,  # +12: el listener de captura honra data-stop — «Agendar» en NO INCLUIDAS abría además la ficha y te dejaba ahí — 30 ago  # antes 1787,  # +7: closeCitySheet al ACTION_REGISTRY — la hoja de ciudad prometía cerrarse tocando el fondo y el registro no la tenía (fallo mudo) — 30 ago  # antes 1780,  # +4: el botón de DESHACER declara su intención (data-restaurar) — usa la misma acción que agendar y sin la marca preguntaría lo que no toca — 26 ago  # antes 1776,  # +4: sameEntry al TEST BRIDGE — el test pregunta al dueño, no reimplementa la identidad — 25 ago  # antes 1772,  # +4: los tres canales vivos también refrescan DATOS (capa 2, live-refresh) — 24 ago  # +15: canales de update fuera del guard de SW + guardián de que no vuelvan (bug iOS sin updates) — 24 ago  # +5: el clic de corto en el palmarés abre su ficha — 24 ago  # +41: canal #4 — poll en primer plano que OFRECE la actualización (doctrina T97) — 24 ago  # +1: accion togglePressScreenings — 23 ago  # +2: acciones openPalmares/closePalmares — 23 ago  # +5: acciones de la hoja de clave de revisión — 23 ago  # +29: vista previa por ?fest= — que el equipo de un festival revise su montaje sin publicarlo — 21 ago
...
# ── [premiere-sin-badge] un distintivo que la tarjeta no sabe resumir ────────
# `premiere` es texto libre: cada festival escribe el suyo («World Premiere»,
# «Estreno Latinoamericano», «Película de Apertura»). La tarjeta lo resume a UNA
# palabra —APERTURA / CLAUSURA / ESTRENO— con las reglas de `premiereBadgeKey`
# en src/domain/film.js, porque los distintivos van cortos (regla de Juan, 23
# sep 2026).
#
# Un valor que no case con ninguna regla NO pierde el dato: la ficha lo sigue
# mostrando entero en su banda de avisos. Pero se queda sin píldora, y eso es
# justo lo que se vuelve invisible sin que nadie se entere — que es como 170
# funciones llevaron su premiere sin pintar desde que se montó Tribeca. Sale
# como AVISO, con el valor y el festival, para añadir la regla o decidir que no
# la merece.
check = 'premiere-sin-badge'
try:
    import json as _jp, glob as _gp, os as _op, re as _rp
    _dom = open('src/domain/film.js', encoding='utf-8').read()
    _bloque = _rp.search(r'_PREMIERE_REGLAS\s*=\s*\[(.*?)\];', _dom, _rp.S)
    if not _bloque:
        raise RuntimeError('no encuentro _PREMIERE_REGLAS en src/domain/film.js — '
                           'si se renombró, este guardián estaría midiendo el aire')
    _reglas = [_rp.compile(_m.group(1), _rp.I)
               for _m in _rp.finditer(r'/([^/]+)/i', _bloque.group(1))]
    if not _reglas:
        raise RuntimeError('_PREMIERE_REGLAS existe pero sin reglas legibles')
    _sin = {}
    for _f in sorted(_gp.glob('festivals/*.json')):
        try:
            _d = _jp.load(open(_f, encoding='utf-8'))
        except Exception:
            continue
    ...
    if _sin:
        for _v, _fs in sorted(_sin.items()):
            warn(check, f'«{_v}» no casa con ninguna regla → sin píldora en la '
                        f'tarjeta ({", ".join(sorted(_fs))})')
    else:
        ok(check, f'todo `premiere` publicado se resume en una palabra '
                  f'({len(_reglas)} reglas)')
except Exception as _e:
    fail(check, f'no se pudo verificar premiere-sin-badge: {_e}')
...
try:
    ...
    _INTERNO_OK = {
        ...
        '_duracion_de':          'de dónde sale la duración de una actividad que el festival no imprime',
    }
...
try:
    ...
    for _f in sorted(_g.glob('festivals/*.json')):
        ...
        # «Volver con las manos vacías» es volver SIN NINGÚN texto. Si TMDB solo
        # tiene la sinopsis en inglés —pasa con estrenos recientes y con cortos—,
        # traerse `synopsis_en` ES la cosecha completa: no hay más que traer. El
        # guardián miraba solo `synopsis` y acusaba de no cosechar a quien había
        # cosechado todo lo que existía. Medido antes de tocarlo: en todo el repo
        # esto excusa exactamente 2 obras, las dos de QAFF, y no afloja en ningún
        # otro festival.
...
try:
    ...
    _DEUDA = {
        ...
        # Los dos de abajo salieron a la luz el 23 sep 2026, cuando este guardián
        # dejó de aceptar un comentario como lector. Estaban emitidos e
        # invisibles; los dos festivales ya terminaron, así que se declaran en
        # vez de inventarles superficie con el festival cerrado.
        'competencia': 'FINCA (13) — en qué competencia oficial va la obra; la app '
                       'muestra la SECCIÓN, que en FINCA no es lo mismo. Sin superficie.',
        'format': 'TIFF (873) — formato de proyección. 871 de 873 son DCP, así que '
                  'un badge sería ruido; lo que valdría es marcar las excepciones '
                  '(un 16mm y un 70mm). Sin superficie.',
    }
    # LOS COMENTARIOS NO SON LECTORES (23 sep 2026). Este guardián buscaba el
    # nombre del campo en el TEXTO PLANO de src/, sin distinguir código de
    # comentario. `premiere` aparecía en un comentario de i18n.js —«Lote 13:
    # cierre completo (prio/conflict/splash/fs/meta/premiere/lang/…»— y con eso
    # se daba por leído. No lo pintaba NADIE: 170 funciones de Tribeca, FINCA,
    # FICMontañas y FICMA llevaban su «World Premiere» y su «Película de
    # Apertura» guardados e invisibles desde que se montaron, con el vigilante
    # diciendo que todo bien. Una nota nuestra no es una pantalla.
    def _sin_comentarios(_txt):
        _txt = _re_c.sub('', _txt)                  # /* … */
        return '\n'.join(_l.split('//')[0] if '//' in _l and '://' not in _l else _l
                          for _l in _txt.split('\n'))
    import re as _re_mod
    _re_c = _re_mod.compile(r'/\*.*?\*/', _re_mod.S)
    _src_all = ''.join(_sin_comentarios(open(_p, encoding='utf-8').read())
